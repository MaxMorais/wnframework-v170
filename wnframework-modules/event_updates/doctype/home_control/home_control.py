...
# ------------------------------------- Home page module details -----------------------------------------
	...
	def get_module_order(self):
		...
		ml = [x for x in sql("select name, module_label, module_seq, is_hidden from [tabModule Def] where docstatus<2 order by module_seq asc, module_label asc") if x[0] in show_list]
