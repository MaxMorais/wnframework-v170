...
class DocType:
	...
	def update_permissions(self,args=''):
		args = eval(args)
		di = args['perm_dict']
		doctype_keys = list(di.keys())	# ['Enquiry','Competitor','Zone','State']
		for parent in doctype_keys:
			for permlevel in list(di[parent].keys()):
				for role in list(di[parent][permlevel].keys()): 
					...
					if role:
						...
						# update
						if exists and (1 in list(pd.values())):
						...
						# new
						elif not exists and (1 in list(pd.values())):
						...
						# delete
						elif exists and (1 not in list(pd.values())):
