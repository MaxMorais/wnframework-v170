...
class DocType:
	...
	def get_default_schedule_date( self, obj):
		for d in getlist( obj.doclist, obj.fname):
			...
			if ltd and obj.doc.transaction_date:
				if 'lead_time_date' in d.fields or obj.doc.doctype == 'Indent':
					d.lead_time_date = cstr(add_days( obj.doc.transaction_date, cint(ltd)))
				if 'prevdoc_docname' not in d.fields or ('prevdoc_docname' in d.fields and not d.prevdoc_docname):
	...
	def get_item_details( self, obj, arg =''):
		...
		if obj.doc.doctype in ['Supplier Quotation', 'Purchase Order', 'Purchase Receipt']:
			...
			ret['import_rate'] = flt(item and flt(item[0]['last_purchase_rate']) or 0) / flt('conversion_rate' in obj.doc.fields and flt(obj.doc.conversion_rate) or 1)
	...
	def validate_for_items(self, obj):
		...
		for d in getlist( obj.doclist, obj.fname):
			...
			if 'prevdoc_docname' in d.fields and d.prevdoc_docname:
			...
			e = [d.schedule_date, d.item_code, d.warehouse, d.uom, 'prevdoc_docname' in d.fields and d.prevdoc_docname or '', 'prevdoc_detail_docname' in d.fields and d.prevdoc_detail_docname or '', 'batch_no' in d.fields and d.batch_no or '']
...
# Validate values with reference document
	...
	def update_ref_doctype_dict(self, curr_qty, curr_doctype, ref_dn, ref_dt, ref_tab_fname, ref_tab_dn, transaction, item_code, is_submit, curr_parent_doctype, curr_parent_name):
		...
		if ref_dn not in self.ref_doctype_dict and transaction in self.update_percent_field:
	...
	def update_prevdoc_detail(self, obj, is_submit):
		...
		for d in getlist(obj.doclist, obj.fname):
			...
			# for payable voucher
			if 'purchase_order' in d.fields and d.purchase_order:
			...
			if 'purchase_receipt' in d.fields and d.purchase_receipt:
