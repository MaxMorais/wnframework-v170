...
class DocType(TransactionBase):
  ...
  def get_indent_details(self):
	...
	if d.item_code:
	  ...
	  d.import_rate = flt(item and flt(item[0]['last_purchase_rate']) or 0) / flt('conversion_rate' in self.doc.fields and flt(self.doc.conversion_rate) or 1)
  ...
  def check_for_stopped_status(self, pc_obj):
	check_list =[]
	for d in getlist(self.doclist, 'po_details'):
	  if 'prevdoc_docname' in d.fields and d.prevdoc_docname and d.prevdoc_docname not in check_list:
  ...
  def check_modified_date(self):
	...
	date_diff = sql("select unixepoch('%s', '%s')" % ( mod_db[0][0],cstr(self.doc.modified)))
