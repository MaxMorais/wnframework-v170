...
class CookieManager:
	def __init__(self):
		import http.cookies
		webnotes.cookies = http.cookies.SimpleCookie()
	...
	def get_incoming_cookies(self):
		...
		if 'HTTP_COOKIE' in os.environ:
			...
			for c in list(webnotes.cookies.values()):
	...
	def set_remember_me(self):
		if webnotes.utils.cint(webnotes.form_dict.get('remember_me')):
			...
			for k in list(webnotes.cookies.keys()):
...
class Session:
	...
	def check_expired(self):
		...
		try:
			webnotes.conn.sql("update from tabSessions where unixepoch('now', lastupdate) > %s SET [status]='Expired'", exp_sec)
		except OperationalError as e:
			if 'no such column' in str(e):
				self.add_status_column()
		
		# clear out old sessions
		webnotes.conn.sql("delete from tabSessions where unixepoch('now', lastupdate) > '72:00:00'")
