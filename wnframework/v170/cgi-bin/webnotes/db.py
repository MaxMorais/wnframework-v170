...
class Database:
	...
	def connect(self):
		...
		"""
		from webnotes.db_functions import register


		self._conn = sqlite3.connect(
			os.environ.get('DB_STORAGE')
		)
	...
	def check_transaction_status(self, query):
		...
		if query and query.strip().lower()=='begin transaction':
	...
	def sql(self, query, values=(), as_dict = 0, as_list = 0, formatted = 0, ignore_no_table = 1, debug=0):
		...
		# execute
		if values!=():
			#print(f'SQL: {query.replace("%s", "?")}\nARGS: {values}')
			self._cursor.execute(query.replace('%s', '?'), values)
