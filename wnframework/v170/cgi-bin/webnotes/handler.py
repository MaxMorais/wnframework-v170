...
if 'cmd' in form_dict and (form_dict.get('cmd')=='reset_password'):
...
elif 'cmd' in form_dict and (form_dict.get('cmd')=='prelogin'):
	webnotes.conn = webnotes.db.Database(use_default = 1)
	sql = webnotes.conn.sql
...
# ---------------------------------------------------------------------
	...
	try:
		...
		if form_dict.get('cmd') != 'login' and webnotes.conn:
			...
			# get command cmd
			cmd = 'cmd' in form_dict and form_dict.get('cmd') or ''
			read_only = '_read_only' in form_dict and form_dict.get('_read_only') or None
			...
			if '.' in cmd:
				...
				exec('from %s import %s' % (module, cmd), locals())			
