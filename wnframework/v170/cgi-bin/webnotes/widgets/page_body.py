...
def load_properties():
	...
	if page_url:
		if page_url.startswith('Page/'): 
			page_url = page_url[5:]
		page_url = ['Page', urllib.parse.unquote(page_url)]
	else:
...
def get():
	"""
	   returns the full rendered index.py
	...
	if webnotes.form_dict.get('sid'): 
		return redirect_template % ('Redirecting...', ('index.py' + (page and ('?page='+page) or '')))
