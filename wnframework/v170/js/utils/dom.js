...
wn.urllib = {
	...
	// returns the base url with http + domain + path (-index.py or # or ?)
	get_base_url: function() {
		var url= window.location.href.split('#')[0].split('?')[0].split('index.py')[0];
	...
	get_file_url: function(file_id) {
		...
		return repl('cgi-bin/getfile.py?name=%(fn)s&acx=%(ac)s', {fn:file_id, ac:ac_id})
	}	
}
...
set_user_img = function(img, username, get_latest, img_id) {
	function set_it(i) {
		if(user_img[username]=='no_img_m')
			...
			i.src = repl('cgi-bin/getfile.py?ac=%(ac)s&name=%(fn)s', {fn:user_img[username], ac:ac_id});			
		}
	}
