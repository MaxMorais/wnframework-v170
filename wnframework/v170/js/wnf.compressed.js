...
return d;},get_base_url:function(){var url=window.location.href.split('#')[0].split('?')[0].split('index.py')[0];if(url.substr(url.length-1,1)=='/')url=url.substr(0,url.length-1)
return url},get_file_url:function(file_id){var ac_id=locals['Control Panel']['Control Panel'].account_id;return repl('cgi-bin/getfile.py?name=%(fn)s&acx=%(ac)s',{fn:file_id,ac:ac_id})}}
...
i.src='images/ui/no_img/no_img_f.gif';else{ac_id=locals['Control Panel']['Control Panel'].account_id;i.src=repl('cgi-bin/getfile.py?ac=%(ac)s&name=%(fn)s',{fn:user_img[username],ac:ac_id});}}
...
var outUrl="index.py";var NULL_CHAR='^\5*';function checkResponse(r,on_timeout,no_spinner,freeze_msg){try{if(r.readyState==4&&r.status==200)return true;else return false;}catch(e){msgprint("error:Request timed out, try again");if(on_timeout)
...
req.onreadystatechange=ret_fn;req.open("POST",'cgi-bin/getjsfile.py',true);req.setRequestHeader("ENCTYPE","multipart/form-data");req.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=UTF-8");req.send(makeArgString({filename:fn}));set_loading();}
